...
class BuildIdentityTests(unittest.TestCase):
    ...
    def test_cmake_agrees_including_case_sensitive_release_flags(self):
        ...
        fragment = 'file(GLOB FREELLM_BUILD_INPUTS'+source.split('file(GLOB FREELLM_BUILD_INPUTS', 1)[1]
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            for name in ('src/qwen/z.cpp', 'include/qwen/a.hpp', 'kernels/metal/qwen.metal',
            ...
            for mode, flags, sanitizer in (('Release', '-O3 -DNDEBUG', 'none'),
                                           ...
                                           ('Debug', '-g', 'address')):
                values = dict(CMAKE_BUILD_TYPE=mode, CMAKE_CXX_FLAGS='-fno-fast-math', FREELLM_SANITIZE=sanitizer,
                ...
                script.write_text(''.join(f'set({k} "{v}")\n' for k, v in setup.items())+fragment+
                                  'file(WRITE "'+str(root/'actual')+'" "${FREELLM_BUILD_FINGERPRINT}")\n')
