...
def q3_decision(report):
    ...
    margin=2.776445105*statistics.stdev(logs)/math.sqrt(5)
    lower,upper=math.exp(mean-margin),math.exp(mean+margin)
    return dict(status='memory_disturbed' if not clean else 'worth_quality_investigation' if upper<=1.03 else
                'operator_regression' if lower>1.03 else 'operator_latency_inconclusive',
                one_token_geomean_ratio=math.exp(mean),one_token_lower_95=lower,one_token_upper_95=upper,ratios=ratios,
...
def run_q3(args):
    ...
    with exp:
        ...
        for split,offset in (('tuning',0),('heldout',512)):
            path=exp.out/(split+'.workload.json');save(path,dict(tokens=source[offset:offset+72],continuation=source[offset+72:offset+80],split=split,source_offset=offset))
