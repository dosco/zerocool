...
def check_configuration(state,config):
    if state.get('decode_submission','immediate')!=config.get('decode_submission','immediate'):
        raise ValueError('Decode submission differs from requested configuration')
    if state.get('memory_pressure',{}).get('policy','observe')!=config.get('memory_pressure_policy','observe'):
        raise ValueError('Memory pressure policy differs from requested configuration')
    ...
    for option,key,default in [('kernel_policy','policy','reference'),('token_tile','token_tile',1),
                               ...
                               ('q4_decode','q4_decode','reference'),('attention_score_tiles','attention_score_tiles','full')]:
