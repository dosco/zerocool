...
def next_experiment(index, selector):
    ...
    if out['status'] != 'measured':
        ...
        if data.get('kind')=='mtp_width_screen_v1':
            ...
            if data.get('stage')=='other-coding-diagnostic' and clean and all(
                    r['component_ms_per_token']['wall']>r['opportunity']['budget_ms_per_token'] for r in out['runs']):
                out.update(
                    hypothesis='The measured fixed widths remain below the target; verification dependencies need attribution before another optimization.',
                    smallest_experiment='Capture a bounded middle-generation target profile using the best measured fixed width for each selected workload.',
                    missing_evidence=['Current target GPU/read dependency traces with exact token coverage',
                                      'Held-out evaluation before an adaptive width policy',
                                      'Five fresh pairs, long-context and sustained qualification'])
