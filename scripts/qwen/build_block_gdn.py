...
CLEAR_CAPTURE = ('verifier_kernels.operator_capture.clear();verifier_kernels.capture_phase.clear();'
    'verifier_kernels.capture_operator.clear();verifier_kernels.capture_layer=-2;')
...
def sources(cfg):
    ...
    result[cfg['generated']] = replace(result[cfg['generated']], '    gpu_.configure(verifier_kernels);',
        '    if(phase_!="decode") {'+CLEAR_CAPTURE+'}\n'
        '    gpu_.configure(verifier_kernels);')
    result[harness] = replace(result[harness], '        if(argc==2 && std::string_view(argv[1])=="--checkpoint-self-test") {',
        '        if(argc==2 && std::string_view(argv[1])=="--capture-config-self-test") {\n'
        '            for(bool decode:{false,true}) {\n'
        '                KernelConfig verifier_kernels;verifier_kernels.profile=decode;\n'
        '                verifier_kernels.operator_capture="fixtures";verifier_kernels.capture_phase="decode";\n'
        '                verifier_kernels.capture_operator="gdn";verifier_kernels.capture_layer=0;\n'
        '                if(!decode) {'+CLEAR_CAPTURE+'}\n'
        '                verifier_kernels.validate();\n'
        '            }\n'
        '            std::cout<<"capture configuration validated\\n";return 0;\n'
        '        }\n'
