...
def admission_failure(raw):
    return raw.get('complete') is False and not raw.get('blocks') and raw.get('error') in (
        'fixed memory admission failed', 'checkpoint workspace not admitted',
        'expert slots exceed admitted capacity or are below 32', 'insufficient currently available memory')
    ...
    with exp:
        ...
        for mode in builder.MODES:
            ...
            try:
                ...
                if path.is_file() and admission_failure(load(path)):
