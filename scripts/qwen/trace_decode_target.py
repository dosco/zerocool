...
if __name__=='__main__':
    ...
    p.add_argument('--current-build',action='store_true',help='Version 2: current scratch configuration and decode-only coverage')
    p.add_argument('--slots',type=int,choices=[1848,1460,1072],default=1848)
    p.add_argument('--prompt-tokens',type=int,choices=[72,2048],default=72)
    args=p.parse_args()
    if args.current_build:
        from stage200 import run_profile
        raise SystemExit(0 if run_profile(args)['complete'] else 2)
