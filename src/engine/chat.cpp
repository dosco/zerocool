...
private:
    ...
    void poll() {
        try {
            ...
            while(!stop_) {
                try {
                    auto reply=http(url+"/zerocool/status","GET","",stop_,{},3000,instance);
                    if(reply.status==409)throw std::runtime_error("server instance changed; quit and reconnect explicitly");
                    if(reply.status!=200)throw std::runtime_error("server does not provide ZeroCool status");
                    auto status=Json::parse(reply.body);
                    if(status.value("version",0)!=1)throw std::runtime_error("unsupported ZeroCool status version");
    ...
    void send() {
        while(!stop_) {
            ...
            try {
                if(work.reset) {
                    auto response=http(url+"/zerocool/session/reset","POST","{}",cancel_,{},3000,instance);
    ...
    bool key(const Event& event) {
        ...
        if(input_text_.starts_with("/save ")) {
            try {
                ...
                save_transcript(input_text_.substr(6),{{"format","zerocool-transcript-v1"},{"model",status_.value("model","")},{"entries",entries}});
    ...
    Element render() {
        ...
        if(lines.empty())lines.push_back(paragraph("ZeroCool · local chat. The model stays in the engine process. Type /help and press Ctrl+D for controls.")|dim);
        ...
        Elements view={text("ZeroCool  "+status_.value("model","")+"  "+url_)|bold,separator(),vbox(std::move(lines))|vscroll_indicator|yframe|flex,separator(),paragraph(terminal_text(footer))};
