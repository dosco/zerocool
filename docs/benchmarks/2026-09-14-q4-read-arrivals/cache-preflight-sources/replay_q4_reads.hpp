...
Json invalidate_q4_range(const std::filesystem::path& path,uint64_t offset,uint64_t bytes) {
    ...
    const uint64_t length=(checked_add(offset-start,bytes)+page-1)/page*page;
    ...
    if(status!=0) {munmap(mapping,length);throw std::runtime_error("record cache invalidation failed");}
    const auto elapsed=monotonic_ns()-started;const auto after=resident_pages();
    require(munmap(mapping,length)==0,"record cache unmap failed");
...
int q4_cache_probe(int argc,char** argv) {
    ...
    for(int layer:{0,16,32,47}) {
        ...
        require(item.at("layer")==layer && item.at("length")==ExpertBytes && item.at("stride")==ExpertStride,"cache probe layout differs");
        ...
        for(const auto& e:fixtures.at("layers").at(std::to_string(layer)).at("experts")) {
            const uint64_t offset=item.at("offset").get<uint64_t>()+e.at("expert").get<uint64_t>()*ExpertStride;
