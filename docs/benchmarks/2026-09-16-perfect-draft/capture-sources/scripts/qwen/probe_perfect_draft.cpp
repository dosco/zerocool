...
int main(int argc,char** argv) {
    ...
    try {
        ...
        check(argc==7,"usage: probe MODEL PREPARED INPUT_JSON REPORT WIDTH timing|validate");
        ...
        options.memory=12*GiB;options.context=8192;options.expert_slots=1072;options.panel=512;options.chunk=128;
        ...
        report.update(Json{{"validation",validation},{"width",width},{"verified_tokens",count},
            ...
            {"configuration",{{"memory_bytes",12*GiB},{"context",8192},{"expert_slots",1072},{"artifact","mixed-4_8bit"},
        ...
        phase(output,"load_model");const auto setup_start=monotonic_ns();
        {
            ...
            check(plan.at("limit_bytes")==12*GiB && plan.at("expert_slots")==1072 && plan.at("panel_tokens")==512,"fixed memory admission failed");
