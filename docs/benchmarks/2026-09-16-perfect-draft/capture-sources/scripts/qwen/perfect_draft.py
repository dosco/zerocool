...
CRITERIA = dict(budget_bytes=BUDGET, expert_slots=1072, context=8192,
    ...
    draft_cost_included=False, assumed_acceptance=1.0)
...
def configuration(width):
    require(width in (1, 2, 4), 'Unsupported verification width')
    return dict(memory_bytes=BUDGET, context=8192, expert_slots=1072, artifact='mixed-4_8bit',
...
def observe(raw, frozen, work, width, validation, input_sha256):
    ...
    require(raw.get('kind') == 'perfect_draft_probe_v1' and raw.get('complete') is True and
            ...
            raw.get('verified_tokens') == total and raw.get('configuration') == configuration(width) and
    ...
    require(plan.get('limit_bytes') == BUDGET and plan.get('expert_slots') == 1072 and
...
def run(output, binary):
    ...
    with exp:
        ...
        freeze(exp, [*proof['files'], *host_probe['files'], exp.out/'host-producer.json',exp.out/'producer.json', PROTOCOL, exp.out/'protocol.md',
