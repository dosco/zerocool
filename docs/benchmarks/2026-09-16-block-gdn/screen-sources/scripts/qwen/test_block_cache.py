...
class BlockCacheTest(unittest.TestCase):
    ...
    def test_missing_reordered_unready_and_changed_events_fail_closed(self):
        ...
        mutations=[lambda e:e.pop(),lambda e:e[20].update(sequence=20),
            ...
            lambda e:next(x for x in e if x['event']=='pin')['detail'].update(pins=True),
            lambda e:next(x for x in e if x['event']=='acquire')['detail'].update(slot=0.0),
    ...
    def test_selection_requires_both_orders_and_joint_memory_headroom(self):
        ...
        self.assertEqual(capture.select(captures(900,900),budgets)['selected_candidate'],dict(policy='slru',slots=1460))
