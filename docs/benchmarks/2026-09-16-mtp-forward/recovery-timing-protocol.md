...
The control was clean; the candidate had process compression.
The existing report remains `resource_blocked` and none of its timing is reused.

Post-run correction: the original note called the first 32KiB observation its
peak. The full report reaches 79,921,152 bytes (76.219MiB). The sealed report and
original protocol copy under `recovery-sources/` are unchanged; this correction
does not qualify its memory or timing.
...
stop if the first candidate is below 5 tokens/s; otherwise finish five pairs.
