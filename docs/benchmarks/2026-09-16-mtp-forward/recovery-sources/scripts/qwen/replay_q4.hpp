...
int replay_q4(int argc,char** argv) {
    ...
    const bool coordinator=condition=="coordinator";
    const bool with_resident=condition=="resident" || coordinator;
    require(condition=="direct" || condition=="scatter" || with_resident,"unknown replay condition");
    require(argc==(with_resident?6:5),"replay FIXTURES REPORT replay direct|scatter|resident|coordinator [MODEL]");
    ...
    struct Fixture { Buf record;std::array<Buf,8> inputs;int layer,expert;std::filesystem::path path; };
    ...
    for(int layer:{0,16,32,47}) {
        ...
        for(const auto& e:item.at("experts")) {
            ...
            fixtures.push_back({record,inputs,layer,e.at("expert").get<int>(),root/e.at("record").at("file").get<std::string>()});
        }
    }
    std::vector<ExpertKey> keys;
    for(const auto& f:fixtures) keys.push_back({uint32_t(f.layer),uint32_t(f.expert)});
    auto fixture_index=[&](ExpertKey key) {
        const auto found=std::find(keys.begin(),keys.end(),key);
        require(found!=keys.end(),"unknown replay expert");return size_t(found-keys.begin());
    };
    std::vector<std::unique_ptr<File>> cache_files;
    std::unique_ptr<ReadPool> reads;std::unique_ptr<ExpertCache> cache;
    if(coordinator) {
        for(const auto& f:fixtures) cache_files.push_back(std::make_unique<File>(f.path));
        reads=std::make_unique<ReadPool>(8);
        cache=std::make_unique<ExpertCache>(8,[&](uint64_t bytes){return gpu.allocate(bytes,AllocationClass::Expert);},
            *reads,[&](ExpertKey key,const Buf& into){
                cache_files[fixture_index(key)]->read(0,{into->data,size_t(ExpertBytes)});
            });
        // Real cache slots and futures, primed before any measured arm. No timed reads.
        for(size_t i=0;i<keys.size();++i) {
            auto lease=cache->acquire(keys[i]);
            require(std::memcmp(lease.wait()->data,fixtures[i].record->data,ExpertBytes)==0,"cached replay record changed");
        }
        reads->drain();
        for(auto& f:fixtures) f.record.reset(); // Cache owns the sole expert payload copy during timing.
        gpu.finish();
    }
    auto io_counters=[&] {
        std::array<uint64_t,2> counters{};
        for(const auto& f:cache_files) {counters[0]+=f->read_calls.load();counters[1]+=f->read_bytes.load();}
        return counters;
    };
    ...
    auto persist=[&](bool complete) {
        const Json report={{"kind","native_q4_replay_v1"},{"complete",complete},{"condition",condition},
            ...
            {"execution_schedule",coordinator?"coordinator-all-hit":"fixed-groups"},
            {"cache_slots",coordinator?8:0},{"io_workers",coordinator?8:0},
    ...
    auto execute=[&](bool candidate,uint32_t group,uint32_t cycles,int validation) {
        ...
        const auto before=gpu.statistics();const auto io_before=io_counters();
        uint64_t coordinator_wait=0,hits=0,misses=0,joins=0;
        ...
        for(uint32_t cycle=0;cycle<cycles;++cycle) for(uint32_t batch=0;batch<8;++batch) {
            ...
            auto encode=[&](size_t i,const Buf& record) {
                const auto& f=fixtures[i];const int position=positions[i];
                encode_expert_rows(gpu,record,f.inputs[batch],output,std::span(&position,1),1,128,
                                   condition=="direct",f.layer,batch);
            };
            if(coordinator) {
                const auto result=execute_experts(keys,*cache,*reads,gpu,group,
                    [&](ExpertKey key,const Buf& record){encode(fixture_index(key),record);});
                coordinator_wait+=result.at("coordinator_wait_ns").get<uint64_t>();
                hits+=result.at("ready_hits").get<uint64_t>();misses+=result.at("new_misses").get<uint64_t>();
                joins+=result.at("loading_joins").get<uint64_t>();
            } else {
                std::deque<std::shared_ptr<Metal::Completion>> pending;
                for(uint32_t at=0;at<8;at+=group) {
                    replay_check();
                    for(uint32_t i=at;i<at+group;++i) encode(i,fixtures[i].record);
        ...
        const auto wall=monotonic_ns()-begin;const auto after=gpu.statistics();const auto io_after=io_counters();
        Json sample={{"expert_executions",uint64_t(cycles)*64},{"wall_ns",wall}};
        if(coordinator) {
            require(hits==uint64_t(cycles)*64 && !misses && !joins && io_after==io_before,"all-hit replay performed a read");
            sample.update({{"coordinator_wait_ns",coordinator_wait},{"ready_hits",hits},{"new_misses",misses},
                {"loading_joins",joins},{"read_calls",io_after[0]-io_before[0]},{"read_bytes",io_after[1]-io_before[1]}});
        }
