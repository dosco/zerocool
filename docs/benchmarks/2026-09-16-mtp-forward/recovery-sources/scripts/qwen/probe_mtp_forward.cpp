...
void joint(const std::filesystem::path& model_path,const std::filesystem::path& prepared,const Json& input,
    ...
    const bool serial=mode=="serial",validation=mode=="validate" || mode=="fast-validate",fast=mode.starts_with("fast-");
    ...
    while(consumed<count) {
        ...
        if(width==4) {
            ...
            if(fast) {
                auto aligned=gpu.allocate(uint64_t(keep)*Hyper*4);
                gpu.copy(seed,0,aligned,0,Hyper*4);
            ...
            if(fast) draft.catch_up(ids,seed,draft_state,&stopped);
