...
def trace_batches(sample, group, hits, records, shared=False, variant=None):
    ...
    for batch in batches:
        ...
        if shared:
            shared_commands.append(shared_trace(batch, batch_commands, variant))
        else:
    ...
    result = dict(captured_batches=8, captured_experts=64, captured_commands=len(commands),
...
def analyze(raw, frozen=None, expected_records=None, expected_output=OUTPUT_SHA, shared=False):
    ...
    for pair in raw['pairs']:
        ...
        for arm in pair['arms']:
            ...
            if mode == 'trace': captures.append(dict(label, **trace_batches(sample, pair['group'], hits, records, shared, arm['variant'])))
