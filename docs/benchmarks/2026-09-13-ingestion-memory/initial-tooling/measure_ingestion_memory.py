...
def expected_boundaries(requests):
...
def analyze_trace(rows,coverage,requests):
    expected=expected_boundaries(requests);detail=sum(not life for life,_ in expected)
    if (coverage.get('kind')!='memory_trace_coverage_v1' or coverage.get('limit')!=512 or
        ...
        coverage.get('lifecycle_records')!=8 or coverage.get('records')!=len(rows) or len(rows)!=len(expected)):
    ...
    for sequence,(row,(life,where)) in enumerate(zip(rows,expected),1):
        ...
        if where['event'] in ('before_model_load','model_destroyed'):
...
def validate(raw,events,evidence,config,workload,expected):
    if raw.get('kind')!='ingestion_memory_diagnostic_v1' or raw.get('status')!='captured':raise ValueError('Missing diagnostic result')
    ...
    return dict(trace=analyze_trace(events,raw['trace_coverage'],raw['runs']),decode=decode,requests=summaries)


def run(out):
    out=out.resolve();out.mkdir(parents=True,exist_ok=False);started=time.monotonic()
    report=dict(kind='ingestion_memory_capture_v1',complete=False,status='running',phase='prepare',measurements=[],
