...
def generated(output):
    ...
    source=replace(source,'model.phase("decode");const auto vstart=monotonic_ns();',
        'model.phase("decode");const auto outside=model.take_profile();\n'
        '        check(outside.at("command_groups").empty() && outside.at("expert_dependencies").empty(),"work escaped target profile window");\n'
        '        const auto target_before=gpu.statistics();const auto vstart=monotonic_ns();')
    source=replace(source,'verify_ns=monotonic_ns()-vstart;',
        ...
        '        auto outside_config=options.kernels;outside_config.profile=false;outside_config.counter_profile=false;gpu.configure(outside_config);')
    source=replace(source,'    const auto request_end=monotonic_ns();',
        '    {const auto outside=model.take_profile();check(outside.at("command_groups").empty() && outside.at("expert_dependencies").empty(),"work escaped final target profile window");}\n'
