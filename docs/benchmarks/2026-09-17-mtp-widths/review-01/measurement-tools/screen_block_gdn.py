    ...
    for case in raw['cases']:
        ...
        for pair in case['pairs']:
            ...
            for arm in pair['arms']:
                require(type(arm['candidate']) is bool and type(arm['repeats']) is int and
                    arm['repeats'] == (1 if validation else 32) and
                    type(arm['gpu_ns']) is int and type(arm['wall_ns']) is int and
                    0 < arm['gpu_ns'] <= arm['wall_ns'], 'Changed arm, repeats or GPU duration')
        ...
        for arm in [*case['warmup'], *[a for p in case['pairs'] for a in p['arms']]]:
            require(arm['exact'] is True and arm['output_sha256'] == case['reference_sha256'] and
                type(arm['gpu_ns']) is int and type(arm['wall_ns']) is int and 0 < arm['gpu_ns'] <= arm['wall_ns'],
