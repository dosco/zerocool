#!/usr/bin/env python3
"""Offline evidence queries plus an explicit, guarded developer trial command."""
import argparse
import datetime
...
def record(index, input_path, directory):
    ...
    if 'tags' in entry:
        tags = entry['tags']
        if (not isinstance(tags, dict) or set(tags)-{'optimization','workload','configuration','rejection_reason'} or
            any(not isinstance(v,list) or len(v)>32 or not all(isinstance(x,str) and 0<len(x)<=160 for x in v) for v in tags.values())):
            raise ValueError('Invalid structured experiment tags')
        extra['tags'] = tags
    if 'rerun_rationale' in entry:
        why = entry['rerun_rationale']
        if not isinstance(why,str) or not 0<len(why.strip())<=4000: raise ValueError('Invalid rerun rationale')
        extra['rerun_rationale'] = why
...
def main(argv=None):
    ...
    for name in ('cycles', 'opportunity'):
        p = subs.add_parser(name, help='Source-bound MTP cycle accounting'); p.add_argument('source')
        if name == 'opportunity':
            p.add_argument('--target-tps', type=float, default=5)
            p.add_argument('--phase', default='recovery')
    p = subs.add_parser('trial', help='Run a bounded registered experiment recipe')
    ...
    try:
        ...
        if args.command == 'import': answer = index.import_paths(args.paths, args.rebuild)
            ...
            from mtp_evidence import cycles
            answer = cycles(index,args.source)
        elif args.command == 'opportunity':
            from mtp_evidence import opportunity
            answer = opportunity(index,args.source,args.target_tps,args.phase)
        elif args.command == 'trial':
            from trial_recovery import run
            answer = run(args.recipe,args.output,args.resume,index)
        ...
        return 2 if answer.get('comparable') is False or (args.command=='trial' and not answer.get('complete')) else 0
