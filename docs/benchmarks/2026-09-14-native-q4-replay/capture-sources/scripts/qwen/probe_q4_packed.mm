...
int main(int argc,const char** argv) {
    @autoreleasepool {
        try {
            check(argc==6,"usage: probe_q4_packed REFERENCE_SHADER CANDIDATE_SHADER FIXTURES REPORT timing|validate[-stream]");
            const std::string mode=argv[5];const bool stream=mode.ends_with("-stream");
            const bool validation=mode=="validate" || mode=="validate-stream";
            check(validation || mode=="timing" || mode=="timing-stream","invalid mode");
...
#pragma clang diagnostic pop
            ...
            auto allocate=[&](uint64_t size) {
                ...
                allocated+=buffer.allocatedSize;check(allocated<=(stream?384ull:64ull)*1024*1024,"allocation bound exceeded");return buffer;
            };
            ...
            id<MTLBuffer> copy_source=nil,copy_output=nil;id<MTLComputePipelineState> copy_pipeline=nil;
            if(stream) {
                copy_source=allocate(128*1024*1024);copy_output=allocate(128*1024*1024);
                copy_pipeline=[device newComputePipelineStateWithFunction:[library newFunctionWithName:@"copy_words"] error:&error];
                check(copy_pipeline!=nil,"stream pipeline unavailable");
                std::memset(copy_source.contents,0x5a,128*1024*1024);
            }
            ...
            auto execute=[&](uint32_t variant,uint32_t group,bool floating,uint32_t cycles) {
                double gpu_us=0,wall_us=0;
                for(uint32_t c=0;c<cycles;++c) for(uint32_t first=0;first<64;first+=group) {
                    @autoreleasepool {
                        if(stream) {
                            auto copy=[queue commandBuffer];auto encoder=[copy computeCommandEncoder];
                            [encoder setComputePipelineState:copy_pipeline];[encoder setBuffer:copy_source offset:0 atIndex:0];
                            [encoder setBuffer:copy_output offset:0 atIndex:1];const uint32_t words=128*1024*1024/4;
                            [encoder setBytes:&words length:sizeof(words) atIndex:2];
                            [encoder dispatchThreads:MTLSizeMake(words,1,1) threadsPerThreadgroup:MTLSizeMake(256,1,1)];
                            [encoder endEncoding];[copy commit];[copy waitUntilCompleted];
                            check(copy.status==MTLCommandBufferStatusCompleted,"stream command failed");
                        }
                        ...
                        wall_us+=std::chrono::duration<double,std::micro>(std::chrono::steady_clock::now()-start).count();
                    }
                }
                return @{@"gpu_us_per_expert":@(gpu_us/(64*cycles)),@"wall_us_per_expert":@(wall_us/(64*cycles))};
            };
            auto exact=[&] {
            ...
            auto report=@{@"kind":@"q4_packed_operator_probe_v1",@"complete":@YES,@"validation":@(validation),
                ...
                @"stream_bytes_before_each_group":@(stream?128*1024*1024:0),@"stream_time_included":@NO,
