...
class PackedQ4EvidenceTest(unittest.TestCase):
    ...
    def test_larger_buffers_require_the_declared_locality_intervention(self):
        raw = self.fixture(); raw['max_shared_buffer_bytes'] = 280*1024**2
        with self.assertRaises(ValueError): analyze(raw)
        raw.update(stream_bytes_before_each_group=128*1024**2, stream_time_included=False)
        self.assertTrue(analyze(raw)['advance_to_request_screen'])
        raw['stream_time_included'] = True
        with self.assertRaises(ValueError): analyze(raw)
