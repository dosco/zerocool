...
CONDITIONS = ('direct', 'scatter', 'resident')
BUDGET = 12*1024**3
...
def analyze(raw, frozen=None):
    ...
    require(uint(resident) and resident == raw.get('expected_resident_bytes') and
            (0 < resident < BUDGET if condition == 'resident' else resident == 0), 'Wrong resident intervention')
    ...
    results = [dict(group=group, metrics={key: metric(samples[group], key) for key in
               ('gpu_command_ns', 'wall_ns', 'cpu_encode_ns', 'cpu_gpu_wait_ns')}) for group in (1, 4)]
    ...
    return dict(status='disturbed' if not clean_all else 'diagnostic_gain' if gain else 'no_clear_native_gain',
...
def run(args):
    ...
    with exp:
        ...
        if args.condition == 'resident': command.append(exp.model)
