...
def query(index,selector):
    ...
    require(data.get('kind')=='mtp_width_target_profile_v1','Select a current-width profile stage')
    ...
    require(proof.get('kind')=='mtp_width_profile_producer_v1' and proof.get('complete') is True and
    ...
    captured=observation(root,raw,reference,read(root/'workload.json'),sha(root/'workload.json'),
        proof['binary_sha256'],identity['build'],identity['artifact_revision'])
    ...
    return dict(status='instrumented_diagnostic',hypothesis=hypothesis,smallest_experiment=experiment,
