...
struct Journal {
    ...
    uint32_t offset=0,recorded_width=4;
    std::array<int,4> ids{};
    ...
    void begin(std::span<const int> tokens,uint32_t at) {
        require(!active && (tokens.size()==2 || tokens.size()==4) && at<=8192-tokens.size(),"invalid recovery capture scope");
        for(auto id:tokens) require(id>=0 && id<Vocab,"invalid journal token");
        std::copy(tokens.begin(),tokens.end(),ids.begin());offset=at;recorded_width=uint32_t(tokens.size());active=true;complete=false;ple_seen=false;
    ...
    void capture(Metal& gpu,int layer,const Buf& convolution,const Buf& normalized,const Buf& a,const Buf& b,
        ...
        require(convolution && normalized && a && b && alog && dt && convolution->bytes==recorded_width*10240*4 &&
            normalized->bytes==convolution->bytes && a->bytes==recorded_width*48*4 && b->bytes==a->bytes && ad<=2 && dd<=2 &&
    ...
    void capture_ple(Metal& gpu,const Buf& x) {
        require(active && !ple_seen && x && x->bytes==uint64_t(recorded_width)*Hyper*4,"invalid PLE capture");
        gpu.copy(x,0,ple,0,x->bytes);ple_seen=true;
    }
    void validate(const State& state,uint32_t keep) const {
        require(complete && !active && ple_seen && (recorded_width==2 || recorded_width==4) && keep>=1 && keep<=recorded_width && state.artifact==Artifact::Mixed,
            "incomplete or incompatible recovery journal");
        for(int l=0;l<Layers;++l) {
            const auto& s=state.layers[l];require(s.position==offset+recorded_width,"journal position differs");
            if((l+1)%4) {
                ...
                require(s.keys && s.values && s.index && (uint64_t(offset)+recorded_width)*512*4<=s.keys->bytes &&
                    s.keys->bytes==s.values->bytes && (uint64_t(offset)+recorded_width)*128*4<=s.index->bytes,"attention recovery bounds");
            }
        }
        require(ple && ple->bytes==4*Hyper*4 && state.valid && state.tokens==offset+recorded_width && state.layers[1].ple_conv &&
    ...
    void apply(Metal& gpu,State& state,uint32_t keep,const std::atomic<bool>* cancel=nullptr) {
        require(complete && !active && !state.valid && keep>=1 && keep<recorded_width && state.tokens==offset,"invalid recovery transaction");
    ...
    Json stats() const {return {{"reserved_bytes",ReserveBytes},{"fixed_allocation_bytes",fixed_bytes},
        {"peak_incremental_bound_bytes",bound_bytes()},{"recorded_width",recorded_width},{"captures",captures},{"repairs",repairs},
