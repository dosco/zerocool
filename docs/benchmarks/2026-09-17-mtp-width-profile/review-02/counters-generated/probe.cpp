...
void joint(const std::filesystem::path& model_path,const std::filesystem::path& prepared,const Json& input,
    ...
    const uint32_t forced_keep=input.value("force_prefix",1u);check(forced_keep>=1 && forced_keep<=requested_width,"invalid forced prefix");
    ...
    report["performance_measurement"]=false;report["profile_workspace_bytes"]=536870912ull;
    ...
    const auto combined=plan.at("planned_bytes").get<uint64_t>()+host_bound+MtpDraft::budget_bytes(32,8192)+mtp_scratch::ReserveBytes+mtp_recovery::ReserveBytes+536870912ull;
    ...
    report["admission"]={{"target",plan},{"draft_slots",32},{"draft_bytes",MtpDraft::budget_bytes(32,8192)},
        {"host_checkpoint_logits_bytes",host_bound},{"combined_bytes",combined},{"profile_workspace_bytes",536870912ull},{"expert_scratch_reserve_bytes",mtp_scratch::ReserveBytes},{"target_recovery_reserve_bytes",mtp_recovery::ReserveBytes}};
    ...
    while(consumed<work.count && !eos) {
        ...
        if(width>1) {
            ...
            const auto save_start=monotonic_ns();checkpoint.save(state,width);draft_checkpoint.save(gpu,draft_state,width);
            ...
            draft_ns=monotonic_ns()-begin;proposed+=width-1;
        }
        const bool forced=work.validation && consumed==0 && width>1 && forced_keep<width && capture_path.empty();
        if(work.validation && consumed==0 && width>1 && capture_path.empty())
        ...
        const bool profiling=consumed>=32 && profile_committed<16;
        ...
        {mtp_recovery::Capture capture(width>1 && (state_only || bundle)?&journal:nullptr,ids,state.tokens);
        ...
        if(width>1) {
            if(keep<width) {
                ...
                if(state_only) {
                    journal.validate(state,keep);checkpoint.check_prefix(state,keep,width);mtp_recovery::cancelled(&stopped);
                    checkpoint.restore_prefix(state,keep,width);target_restore_ns=monotonic_ns()-begin;
        ...
        report["cycles"].push_back({{"request_id",report.at("request_id")},{"cycle_id",report["cycles"].size()},
            ...
            {"target_recovery_read_bytes",target_recovery_read_bytes},{"target_recovery_forward_calls",target_recovery_forward_calls},{"offset",offset},{"width",width},{"proposals",ids},{"accepted_proposals",good},
        ...
        if(profiling) {
            ...
            check(profile_calls<=16 && profile_committed<=19,"profile coverage exceeded bound");
            size_t entries=0;for(const auto& g:profile.at("command_groups")) entries+=g.at("operations").size();
            check(entries>0 && entries<=20000 && profile.at("truncated")==false &&
    ...
    check(profile_calls>0 && profile_committed>=16 && consumed==128 && !eos,"incomplete middle-generation profile");
    report["profile_window"]={{"requested_start",32},{"requested_committed",16},
...
int main(int argc,char** argv) {
    Json report={{"kind","native_mtp_width_counter_profile_v1"},{"complete",false},{"production_promoted",false}};bool write=false;
    try {
