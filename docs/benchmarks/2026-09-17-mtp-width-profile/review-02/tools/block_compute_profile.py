...
def analyze_block(block, profile, build, revision, mode, *, tokens=4, expert_geometry='scatter', entry_limit=100000):
    require(type(tokens) is int and tokens in (1,2,4) and
        expert_geometry in ('scatter','down_projection') and
        (expert_geometry!='scatter' or tokens==4), 'Unsupported expert geometry')
    ...
    require(profile.get('truncated') is False and profile.get('coverage') == 'all-dispatches' and
        profile.get('entry_limit') == entry_limit and profile.get('dependency_capture_limits') ==
    ...
    outputs_by_layer = defaultdict(Counter); scatters_by_layer = defaultdict(Counter)
    for g in groups:
        ...
        for op in ops:
            require(op['request_phase'] == 'decode' and op['offset'] == offset and
                ...
                type(op['tokens']) is int and 1 <= op['tokens'] <= tokens and
            ...
            if op['stage'] == 'routed_expert':
                if op['kernel']=='scatter_experts':
                    scatters_by_layer[op['layer']][op['tokens']]+=1
                    if expert_geometry=='scatter':expert_rows[op['tokens']]+=1
                if expert_geometry=='down_projection' and matrix.get('K')==640 and matrix.get('N')==2560:
                    require(matrix.get('bits')==4 and matrix.get('group')==64 and matrix.get('quantized') is True and
                        matrix.get('rows')==op['tokens'] and op['kernel'].startswith('q4_mm'), 'Changed expert output projection')
                    expert_rows[op['tokens']]+=1;outputs_by_layer[op['layer']][op['tokens']]+=1
            if mode == 'dispatch':
    ...
    for d in deps:
        require(d['offset'] == offset and d['tokens'] == tokens and d['request_phase'] == 'decode' and
            d['build'] == build and d['artifact_revision'] == revision, 'Dependency identity differs')
        route = d['routes']; require(len(route) == tokens*10 and all(type(e) is int and 0 <= e < 512 for e in route) and
            all(len(set(route[t:t+10])) == 10 for t in range(0, tokens*10, 10)), 'Invalid top-ten routes')
        if expert_geometry=='down_projection':
            expected=Counter(Counter(route).values())
            scatter_expected=Counter({n:c for n,c in expected.items() if tokens!=4 or n!=1})
            require(outputs_by_layer[d['layer']]==expected and scatters_by_layer[d['layer']]==scatter_expected,
                    'Expert output/scatter geometry differs from selected routes')
    ...
    require(sum(expert_rows.values()) == sum(reads.values()) and
        sum(rows*count for rows, count in expert_rows.items()) == 48*tokens*10,
    ...
    operation_rows = [dict(stage=k[0], kernel=k[1], K=k[2], N=k[3], rows=k[4],
        gpu_pass_ms_per_token=v[0]/(tokens*1e6), dispatches=v[1]) for k, v in
        sorted(operations.items(), key=lambda kv: -kv[1][0])]
    return dict(offset=offset, tokens=tokens, forward_ms_per_token=(end-begin)/(tokens*1e6),
        buckets_ms_per_token={k: v/(tokens*1e6) for k, v in buckets.items()},
        dispatches=sum(kernels.values()), kernels=dict(kernels), command_groups=len(groups),
        classes=command_classes(groups, tokens), layers=per_layer, routes=[list(row) for row in sorted(routes)],
