...
def decode(data,raw,work,fingerprint):
    ...
    for event in events[1:-1]:
        ...
        if name=='forward_begin':
            ...
            require(d==dict(offset=at,tokens=count,input=tokens[at:at+count],phase=phase),'Changed forward coverage or tokens')
            ...
            state=cache.state();require(d['state']==state,'Native cache state does not replay')
            digest=hashlib.sha256(json.dumps(state,separators=(',',':')).encode()).hexdigest();stats=d['stats']
