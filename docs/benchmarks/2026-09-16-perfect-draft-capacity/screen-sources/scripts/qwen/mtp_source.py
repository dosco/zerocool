...
def write_all(stream, data):
    """FileIO writes may be short; never publish a partially written payload."""
    remaining = memoryview(data).cast('B')
    while remaining:
        count = stream.write(remaining)
        require(type(count) is int and 0 < count <= len(remaining), 'Short source/prepared write')
...
def validate_inventory(data):
    ...
    # The API response includes live downloads/likes even at a pinned revision.
    # Preserve its local hash, but pin only immutable checkpoint identity here.
    def checkpoint_identity(value):
        metadata=dict(value['metadata_sha256'])
        observed=metadata.pop('hub-metadata.json')
        require(isinstance(observed,str) and len(observed)==64 and
            all(c in '0123456789abcdef' for c in observed),'Invalid Hub metadata checksum')
        return dict(value,metadata_sha256=metadata)
    require(checkpoint_identity(data)==checkpoint_identity(read_json_bytes(lock.read_bytes())),
        'MTP inventory differs from the pinned source lock')
...
def download(directory):
    ...
    for position,(name,tensor) in enumerate(sorted(inventory['tensors'].items())):
        ...
        if saved:
            require(saved.get('source')==tensor and saved.get('file')==str(path.relative_to(directory)) and
                saved.get('bytes')==tensor['bytes'] and not path.is_symlink() and
                path.stat().st_size==tensor['bytes'] and digest_file(path)==saved['sha256'], 'Changed completed source tensor')
            continue
        ...
        with temp.open('wb',buffering=0) as stream:
            ...
            while done<tensor['bytes']:
                ...
                receipt['application_read_bytes']+=len(raw)
                atomic_json(receipt_path,receipt)
                write_all(stream,raw);h.update(raw);done+=len(raw)
