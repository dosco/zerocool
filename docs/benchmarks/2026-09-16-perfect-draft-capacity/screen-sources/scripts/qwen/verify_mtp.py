...
def clean_memory_observations(memory):
    counters=('compressed_bytes','compressed_peak_bytes','decompressions','system_swap_used_bytes')
    return (len(memory)==3 and
        all(type(m.get(k)) is int and m[k]>=0 for m in memory for k in counters) and
        all(m[k]==0 for m in memory for k in counters[:-1]) and
        len({m['system_swap_used_bytes'] for m in memory})==1)
...
def run(prepared,binary,output):
    ...
    files=[Path(__file__),ROOT/'scripts/qwen/prepare_mtp.py',ROOT/'scripts/qwen/mtp_source.py',ROOT/'mtp-models.lock.json',binary,
        *[ROOT/'scripts/qwen'/p for p in ('prepare_storage.py','verify_checkpoint.py','build_identity.py','qualification_evidence.py')],
    ...
    try:
        ...
        clean=clean_memory_observations(memory)
