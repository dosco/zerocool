...
Json Metal::take_profile() {
    reap();auto result=Json{{"command_groups",std::move(impl_->profile)},{"truncated",impl_->profile_truncated},
        {"coverage",impl_->config.profile_decode_only?"decode-only":"all-dispatches"},
...
void Metal::dispatch(const std::string& name,std::initializer_list<Binding> buffers,
    ...
    @autoreleasepool {
        ...
        if(!p.current) p.current=[p.queue commandBuffer];
        if(p.config.counter_profile) {
            ...
            if(p.sample_count>4094 || p.profile_entries>=100000)
...
void Metal::capture_linear(const Linear& l,const Linear* up,const Buf& x,uint32_t tokens,const Buf& rows) {
    ...
    if(dir.empty() || !l.quantized || p.capture.size()>=32) return;
    if((!p.config.capture_phase.empty() && p.request_phase!=p.config.capture_phase) ||
       (!p.config.capture_operator.empty() && p.context.value("stage","")!=p.config.capture_operator) ||
       (p.config.capture_layer!=-2 && p.context.value("layer",-1)!=p.config.capture_layer)) return;
    Json matrix={{"K",l.input},{"N",l.output},{"rows",tokens},{"group",l.group},{"bits",l.bits},
        {"fused",bool(up)},{"gathered",bool(rows)}};
    const auto key=p.request_phase+"/"+p.context.value("stage","")+"/"+std::to_string(p.context.value("layer",-1))+"/"+matrix.dump();if(p.captured_shapes.contains(key)) return;
    const uint64_t weight_bytes=uint64_t(l.input)*l.output/(8/l.bits),meta_bytes=uint64_t(l.input/l.group)*l.output*2;
    const auto needed=(weight_bytes+2*meta_bytes)*(up?2:1)+x->bytes+(rows?rows->bytes:0);
    if(needed>256*MiB-p.captured_bytes) return;
    finish();
    if(p.capture.empty()) {
        if(std::filesystem::exists(dir) && !std::filesystem::is_empty(dir)) throw std::runtime_error("operator capture directory must be new or empty");
        std::filesystem::create_directories(dir);
    }
    Json tensors=Json::object();
    auto save=[&](const char* name,const Binding& binding,uint64_t bytes) {
        const auto filename=std::to_string(p.capture.size())+"-"+name+".bin";
        auto data=binding.buffer->data+binding.offset;
        std::ofstream f(dir/filename,std::ios::binary);f.write(reinterpret_cast<const char*>(data),std::streamsize(bytes));
        if(!f) throw std::runtime_error("cannot save operator capture");
        unsigned char hash[CC_SHA256_DIGEST_LENGTH];CC_SHA256(data,CC_LONG(bytes),hash);
        std::string hex;constexpr char digits[]="0123456789abcdef";for(auto c:hash){hex+=digits[c>>4];hex+=digits[c&15];}
        tensors[name]={{"file",filename},{"bytes",bytes},{"sha256",hex}};
    };
    save("w",l.weight,weight_bytes);save("s",l.scales,meta_bytes);save("b",l.biases,meta_bytes);
    if(up) {save("uw",up->weight,weight_bytes);save("us",up->scales,meta_bytes);save("ub",up->biases,meta_bytes);}
    save("x",{x},x->bytes);if(rows) save("rows",{rows},rows->bytes);
    p.capture.push_back({{"matrix",matrix},{"phase",p.request_phase},{"context",p.context},{"tensors",tensors}});
    p.captured_shapes.insert(key);p.captured_bytes+=needed;
    Json manifest={{"kind","captured_affine_operators"},{"build_fingerprint",BuildFingerprint},
        {"artifact_revision",p.config.artifact_revision},{"bytes",p.captured_bytes},{"byte_limit",256*MiB},{"case_limit",32},
        {"normal_request_latency_qualified",false},{"filter",p.config.json()["capture_filter"]},{"cases",p.capture}};
    std::ofstream file(dir/"manifest.json");file<<manifest.dump(2)<<'\n';if(!file) throw std::runtime_error("cannot save operator manifest");
}
...
void Metal::linear_into(const Linear& l,const Buf& x,uint32_t tokens,Binding out,bool float_output) {
    ...
    if(l.quantized && l.bits==8 && tokens==1 && impl_->config.q8_decode_rows && l.weight.offset%4==0) {
