...
void capture(int argc,char** argv) {
    ...
    Json manifest={{"kind","q3_probe_capture_v1"},{"complete",false},{"origin","existing-Q4-experts"},
    ...
    try {
        Model model(o);Tokenizer tokenizer(o.model);Session session(model,tokenizer);model.prepare_pipelines();
        check(model.memory_plan().limit==12*GiB && model.memory_plan().slots==slots,"Q3 capture memory admission differs");
        o.max_tokens=9;const auto generated=session.generate(tokens,o,{},&stopped);
        check(!stopped && generated.tokens.size()==9,"incomplete Q3 capture request");
...
void probe(char** argv) {
    ...
    check(manifest.at("kind")=="q3_probe_capture_v1" && manifest.at("complete")==true && manifest.at("layers").size()==4,"incomplete capture");
