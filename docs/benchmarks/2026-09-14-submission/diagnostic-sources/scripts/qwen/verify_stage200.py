...
def verify_sources(directory,frozen):
    ...
    paths=sorted([*(root/'src/qwen').glob('*'),*(root/'include/qwen').glob('*')])
    paths += [root/p for p in ('kernels/metal/qwen.metal','models.lock.json','mixed-models.lock.json','mixed-payload-reuse.lock.json','cmake/qwen.cmake')]
    versions={}
    for mode in ('current','initial'):
        lines=[]
        for p in paths:
            relative=p.relative_to(root);saved=directory/'initial-sources'/relative
...
def verify(directory):
    ...
    for path in sorted(directory.iterdir()):
        ...
        row=dict(source=path.name,seal_sha256=digest,recorded_status=report['status'],
                 recorded_complete=report['complete'],build=frozen['build'],source_provenance=verify_sources(directory,frozen))
