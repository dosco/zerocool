...
def validate_pressure_state(state,initial,policy):
    pressure=state.get('memory_pressure',{})
    if pressure.get('policy')!=policy or pressure.get('source')!='dispatch-memorypressure':raise ValueError('Missing pressure policy identity')
    events=pressure.get('events',[])
    if pressure.get('event_count')!=len(events):raise ValueError('Pressure event coverage incomplete')
    current=initial['expert_slots'];shrinks=0
    for i,e in enumerate(events,1):
        level=e.get('level');target=current
        if policy=='shrink' and current>32:
            if level==2:target=max(32,current//2)
            elif level==1:target=max(32,current-388)
        if (level not in (0,1,2) or e.get('sequence')!=i or e.get('before_slots')!=current or
            e.get('requested_slots')!=target or e.get('achieved_slots')!=target or
            type(e.get('released_bytes')) is not int or not 0<=e['released_bytes']<=(current-target)*2768896 or
            type(e.get('drain_duration_ns')) is not int or e['drain_duration_ns']<0):raise ValueError('Invalid pressure transition')
        shrinks+=target<current;current=target
    plan=state['memory_plan'];expected=dict(initial,expert_slots=current,expert_bytes=current*2768896,
        planned_bytes=initial['planned_bytes']-(initial['expert_slots']-current)*2768896)
    if plan!=expected or state['phase_memory']['pressure_resizes']!=shrinks:raise ValueError('Unexplained pressure resize or allocation change')


def validate_request(raw,evidence,config,workload,expected,*,instrumented=False,gpu_reference='off',capacity_axis=False,memory_budget_bytes=12*1024**3,output_tokens=33,pressure_axis=False):
    if pressure_axis and (capacity_axis or config.get('memory_pressure_policy') not in ('observe','shrink')):
        raise ValueError('Pressure and capacity are separate experiment axes')
    ...
    for i,(row,task) in enumerate(zip(raw['runs'],workload)):
        ...
        for state in [row['before'],row['after'],*[s[k] for s in row['phases'].values() for k in ('before','after')]]:
            ...
            if pressure_axis:validate_pressure_state(state,plan,config['memory_pressure_policy'])
            if ((not pressure_axis and (state['memory_plan']!=plan or state['phase_memory']['pressure_resizes']!=0)) or state['diagnostic_stream_trunk'] or
                not state['completion_pipeline'] or
