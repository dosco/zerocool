...
def validate_requests(directory,pairs):
    ...
    require(all(report.get(k)==v for k,v in decision.items()),'Changed prerequisite decision')
    return report
...
def confirmation_decision(rows):
    ...
    passed=(decision['candidate_for_later_qualification'] and all(r.get('clean_memory') is True for r in rows) and
            all(statistics.median(s)>=20 for s in savings))
    return dict(decision,status='qualified_short_gain' if passed else 'improvement_not_demonstrated',
...
def run(args):
    ...
    cs=configs();exp=Experiment(args.output,'expert_residency_'+args.mode+'_v1',cs,prior['workload'],400 if args.mode=='state' else 900)
    with exp:
