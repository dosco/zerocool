...
void encode_expert_rows(Metal& gpu,const Buf& record,const Buf& input,const Buf& output,
    ...
    for(size_t at=0;at<positions.size();at+=chunk) {
        ...
        if((tokens==1 && direct) || direct_single) gpu.linear_into(expert_linear(record,2),activated,1,{output,uint64_t(pos[0])*Hidden*4});
        else {
...
Json execute_experts(std::span<const ExpertKey> selected, ExpertCache& cache,
    ...
    if(coalesce_reads && (tail || encode_group || selected.size()>std::min<size_t>(32,cache.capacity())))
        throw std::invalid_argument("coalesced reads require one admitted window and reference expert encoding");
    ...
    try {
        while(next<selected.size() || !waiting.empty() || !groups.empty()) {
            ...
            // another record, and the ordinary cancellation/drain path applies.
            const bool may_encode=!coalesce_reads || group_sequence==0 ||
                std::all_of(waiting.begin(),waiting.end(),[](const auto& w){return w.lease.ready();});
            if(groups.size()<2 && may_encode) {
                auto& group=encoding;
                for(size_t i=0;i<waiting.size() && group.work.size()<(coalesce_reads?window:group_size);) {
                    if(cancel && cancel->load()) throw std::runtime_error("generation cancelled");
