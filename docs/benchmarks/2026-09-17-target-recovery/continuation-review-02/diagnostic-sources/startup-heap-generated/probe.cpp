...
void joint(const std::filesystem::path& model_path,const std::filesystem::path& prepared,const Json& input,
    ...
    const char* relief = std::getenv("FREELLM_STARTUP_HEAP_RELIEF");
    ...
    model.prepare_pipelines();heap_boundary("loaded");int next=-1;Buf seed;const auto prime_start=monotonic_ns();
...
int main(int argc,char** argv) {
    Json report={{"kind","native_mtp_continuation_v2"},{"complete",false},{"production_promoted",false}};bool write=false;
    try {
