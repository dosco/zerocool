...
out = ROOT/'docs/benchmarks/2026-09-17-target-recovery/startup-heap-01'
...
candidate = ROOT/'.cache/recovery-startup-relief-build-01'
proof = trial.read(candidate/'producer.json')
exp = Experiment(out, 'mtp_startup_heap_diagnostic_v1',
...
with exp:
    ...
    exp.report.update(performance_measurement=False, compression_limit_bytes=512*1024**2,
                      controlled_change='Return unused malloc pages after loading pipelines and after priming', samples=[])
    values=[]
    for arm in ('off','on'):
        trial.host_check(exp, host, arm)
        exp.env['FREELLM_STARTUP_HEAP_RELIEF'] = arm
        ...
        trial.require(raw['startup_heap_relief']==arm and [v['phase'] for v in raw['startup_heap']]==['loaded','primed'],
        ...
        memory=[raw['before_load'],raw['before']['process'],raw['after']['process'],raw['after_destroy'],
                ...
                *[c[k] for c in raw['startup_heap'] for k in ('before','after')]]
    ...
    trial.require(all(values[0][k]==values[1][k] for k in ('row_logits_sha256','committed_token_ids','final_draft_state')),
                  'Heap release changed exact computation')
