...
from perfect_draft import (BUDGET, FLAGS, KERNEL_POLICY, ORDER, VOCAB,
                           compare, configuration, decide, observe,check_prime_reference)
...
def fixture(width=1, validation=False):
    ...
    before = dict(memory_plan=plan, artifact_revision='revision', process=memory,completion_pipeline=True,
...
class PerfectDraftTest(unittest.TestCase):
    def test_setup_change_preserves_prior_prime_math_but_not_old_cache_hash(self):
        original=observation(1)['prime'];changed=copy.deepcopy(original)
        changed['cache_state']=digest('different setup schedule')
        check_prime_reference(changed,original)
        for key in ('logits_sha256','state','routes'):
            bad=copy.deepcopy(changed);bad[key]='changed'
            with self.assertRaises(ValueError):check_prime_reference(bad,original)
    ...
    def test_missing_memory_work_identity_and_counts_fail_closed(self):
        mutations = [lambda r:r.update(input_sha256='changed'),
            ...
            lambda r:r.update(prime_expert_schedule='unspecified'),
            lambda r:r['before'].update(completion_pipeline=False),
            lambda r:r['after'].update(completion_pipeline=False),
