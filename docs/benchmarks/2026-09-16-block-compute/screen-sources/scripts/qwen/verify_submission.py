...
from confirm_expert_residency import validate_requests, validate_state, confirmation_decision
...
def verify(directory):
    ...
    state_path=directory/'residency-state-01';confirm_path=directory/'residency-confirm-01'
    verify_seal(state_path,sha(state_path/'evidence-files.json'))
    state=read(state_path/'summary.json');frozen=read(state_path/'identity.json')
    require(state.get('complete') is True and state['identity']['build']==frozen['build'],'Incomplete state proof')
    proof=validate_state([read(state_path/f'state-{arm}.json') for arm in ('control','candidate')],frozen['build'])
    require(state['correctness']==proof,'Changed full-model proof')
    results.append(dict(source=state_path.name,recorded_status=state['status'],recorded_complete=True,correctness=proof,
        source_provenance=verify_sources(directory,frozen,directory/'qualification-sources')))
    confirmation=validate_requests(confirm_path,5);frozen=read(confirm_path/'identity.json')
    require(confirmation['identity']==state['identity'] and confirmation['correctness']==proof,'Incompatible confirmation identity/state')
    derived=confirmation_decision(confirmation['measurements'])
    require(derived==read(directory/'confirmation-decision.json'),'Changed corrected confirmation decision')
    results.append(dict(source=confirm_path.name,recorded_status=confirmation['status'],recorded_complete=True,derived_decision=derived,
        source_provenance=verify_sources(directory,frozen,directory/'qualification-sources')))
    return dict(kind='submission_experiment_audit_v1',complete=True,audit_passed=True,experiments=results,
        ...
        limitations=['Raw controls, native source identities and measurement arithmetic are reconstructed; inference is not rerun.',
            ...
            'Five fresh pairs and exact native state checks support a short-workload gain, but the 20ms/token append-stage gate was missed.',
            'No long-context or sustained-coding qualification was run.'])
