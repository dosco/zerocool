...
from confirm_expert_residency import confirmation_decision, validate_state
...
class SubmissionTest(unittest.TestCase):
    ...
    def test_confirmation_needs_five_fresh_clean_pairs_and_state_proof(self):
        rows=[dict(pair=p,configuration=arm,clean_memory=True,requests=[dict(request_ms=20000 if arm=='control' else 19000,
            time_to_first_token_ms=10000,decode_wall_ms=10000 if arm=='control' else 9000)]*2) for p,arm in order(5)]
        self.assertTrue(confirmation_decision(rows)['short_gain_qualified'])
        with self.assertRaises(ValueError):confirmation_decision(rows[:-1])
        rows[0]['clean_memory']=False;self.assertFalse(confirmation_decision(rows)['short_gain_qualified'])
        with self.assertRaises(ValueError):validate_state([], 'missing')

    def test_measured_gain_below_target_is_not_labeled_no_improvement(self):
        rows=[dict(pair=p,configuration=arm,clean_memory=True,requests=[dict(request_ms=20000 if arm=='control' else 19000,
            time_to_first_token_ms=10000,decode_wall_ms=10000 if arm=='control' else 9550)]*2) for p,arm in order(5)]
        r=confirmation_decision(rows)
        self.assertTrue(r['request_gain_demonstrated']);self.assertFalse(r['stage_target_met'])
        self.assertEqual(r['status'],'gain_below_stage_target')
